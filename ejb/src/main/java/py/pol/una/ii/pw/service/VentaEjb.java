...
public class VentaEjb {
	...
	public Respuesta<Venta> alta(VentaDto ventaDto) {
		...
		try {
			...
			r.setData(null);
			r.setMessages("La venta no se puede persistir");
			r.setReason("No hay stock suficiente");
	...
	private List<Venta> findAll() {
		/*
		 * TypedQuery<Venta> query = em.createQuery( "SELECT c FROM Venta c",
		 * Venta.class); List<Venta> e = query.getResultList(); if(e.size() > 0)
		 * { return e; }
		 */
		return null;
	}
	...
	private Venta nuevaVenta(VentaDto ventaDto) throws NoExisteElProductoException {
